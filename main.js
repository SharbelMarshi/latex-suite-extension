...
function buildRtlFontCss(hebrewFont, arabicFont) {
  const stack = [hebrewFont, arabicFont].map((font) => font.trim().replace(/["\\]/g, "")).filter(Boolean).map((font) => `"${font}"`).join(", ");
  if (!stack)
    return "";
...
var MathJaxBidiCommandPatcher = class {
  ...
  destroy() {
    ...
    (_c = this.styleEl) == null ? void 0 : _c.remove();
    this.styleEl = void 0;
  }
  patchStyles() {
    var _a;
...
var LatexSuiteExtensionSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Flicker-free inline math").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Disable in tables").setDesc("If turned on, braces won't be inserted in tables. Decorations & atomic ranges are enabled regardless of this setting.").addToggle((toggle) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Disable when using IME input").setDesc("This option can be helpful for avoiding some strange behavior occurring when using IME inputs after escaping from a math block with the Latex Suite plugin's tabout feature.").addToggle((toggle) => {
      toggle.setValue(this.plugin.settings.disableOnIME).onChange(async (disable) => {
        this.plugin.settings.disableOnIME = disable;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Math rendering").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Display style for inline math").setDesc(createFragment((el) => {
      ...
      toggle.setValue(this.plugin.settings.inlineDisplaystyle).onChange(async (enable) => {
        this.plugin.settings.inlineDisplaystyle = enable;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Built-in snippets").setDesc(createFragment((el) => {
      ...
      toggle.setValue(this.plugin.settings.builtinSnippets).onChange(async (enable) => {
        this.plugin.settings.builtinSnippets = enable;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian2.Setting(containerEl).setName("Solve").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Result separator").setDesc(createFragment((el) => {
      ...
      (text) => text.setPlaceholder("\\quad \\to \\quad").setValue(this.plugin.settings.solveSeparator).onChange((0, import_obsidian2.debounce)(
        async (val) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("RTL math text").setHeading();
    ...
    new import_obsidian2.Setting(containerEl).setName("RTL text commands").setDesc(createFragment((el) => {
      ...
      (text) => text.setPlaceholder("e.g. he, ar").setValue(this.plugin.settings.rtlTextCommands.join(", ")).onChange((0, import_obsidian2.debounce)(
        async (val) => {
          this.plugin.settings.rtlTextCommands = val.split(",").map((s) => s.trim()).filter(Boolean);
          await this.plugin.saveSettings();
    ...
    new import_obsidian2.Setting(containerEl).setName("Hebrew font").setDesc("Font used for Hebrew text inside math. The font must be installed on your system (or provided by an Obsidian theme/snippet).").addText(
      (text) => text.setPlaceholder(DEFAULT_HEBREW_FONT).setValue(this.plugin.settings.hebrewFont).onChange((0, import_obsidian2.debounce)(
        async (val) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Arabic font").setDesc("Font used for Arabic text inside math. Leave empty to use the default math text font.").addText(
      (text) => text.setPlaceholder("e.g. Amiri").setValue(this.plugin.settings.arabicFont).onChange((0, import_obsidian2.debounce)(
        async (val) => {
    ...
    if (import_obsidian2.Platform.isDesktopApp) {
      new import_obsidian2.Setting(containerEl).setName("Auto-switch keyboard input").setDesc('Switch the system keyboard by cursor position: the note language outside math, the math language inside $...$, and Hebrew/Arabic inside \\he{...} / \\ar{...}. Requires the im-select command-line tool. Tip: bind the "Toggle keyboard auto-switching" command to a hotkey to pause this quickly.').addToggle((toggle) => {
        toggle.setValue(this.plugin.settings.autoSwitchKeyboard).onChange(async (enable) => {
          this.plugin.settings.autoSwitchKeyboard = enable;
          await this.plugin.saveSettings();
        });
      });
      new import_obsidian2.Setting(containerEl).setName("im-select path").setDesc("Leave empty to auto-detect (/opt/homebrew/bin/im-select or /usr/local/bin/im-select).").addText(
        (text) => text.setPlaceholder("auto-detect").setValue(this.plugin.settings.imSelectPath).onChange((0, import_obsidian2.debounce)(
          async (val) => {
      ...
      new import_obsidian2.Setting(containerEl).setName("Hebrew input source").setDesc("Keyboard layout ID used inside \\he{...}. Run im-select in a terminal with the layout active to print its ID.").addText(
        (text) => text.setPlaceholder("com.apple.keylayout.Hebrew").setValue(this.plugin.settings.hebrewInputSource).onChange((0, import_obsidian2.debounce)(
          async (val) => {
      ...
      new import_obsidian2.Setting(containerEl).setName("Arabic input source").setDesc("Keyboard layout ID used inside \\ar{...}.").addText(
        (text) => text.setPlaceholder("com.apple.keylayout.Arabic").setValue(this.plugin.settings.arabicInputSource).onChange((0, import_obsidian2.debounce)(
          async (val) => {
      ...
      new import_obsidian2.Setting(containerEl).setName("Math input source").setDesc("Keyboard layout ID used inside $...$ (outside \\he / \\ar). Leave empty to not switch when entering math.").addText(
        (text) => text.setPlaceholder("com.apple.keylayout.ABC").setValue(this.plugin.settings.mathInputSource).onChange((0, import_obsidian2.debounce)(
          async (val) => {
      ...
      new import_obsidian2.Setting(containerEl).setName("Note input source").setDesc("Keyboard layout ID for regular note text, applied when leaving math \u2014 the main language of your notes. Leave empty to not switch outside math.").addText(
        (text) => text.setPlaceholder("com.apple.keylayout.Hebrew").setValue(this.plugin.settings.noteInputSource).onChange((0, import_obsidian2.debounce)(
          async (val) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Debug mode").setHeading();
    new import_obsidian2.Setting(containerEl).setName("Disable decorations").setDesc("If turned on, decorations to hide braces adjacent to dollar signs are disabled. This is especially useful when you want to see what this plugin does under the hood.").addToggle((toggle) => {
...
var tolerantMathPostProcessor = (element) => {
  ...
  for (const textNode of textNodes) {
    ...
    while ((match = CANDIDATE_REGEX.exec(text)) !== null) {
      if (!shouldTolerantRender(match[1]))
        continue;
      fragment != null ? fragment : fragment = document.createDocumentFragment();
...
var KeyboardSwitcher = class {
  ...
  exec(bin, args) {
    return new Promise((resolve, reject) => {
      ...
      execFile(bin, args, { timeout: 2e3 }, (error, stdout) => {
        if (error)
          reject(error);
        else
...
function evaluateLatex(latex) {
  const src = latex.replace(/\\left\b|\\right\b/g, " ").replace(/\\[,:;!]|\\ /g, " ").replace(/~/g, " ").replace(/\\cdot\b|\\times\b/g, "*").replace(/\\div\b/g, "/").replace(/\\[dt]frac\b/g, "\\frac");
  const toks = [];
  let i = 0;
  while (i < src.length) {
    const c = src[i];
  ...
  const parseAtom = () => {
    ...
    if (t.t === "cmd") {
      ...
      if (t.v in FUNCTIONS)
        return FUNCTIONS[t.v](parseUnary());
      fail();
    }
    if (t.t === "ch") {
      if (t.v === "(") {
...
function findTopLevel(segment, ch) {
  const positions = [];
  let depth = 0;
  let i = 0;
...
function formatValue(v) {
  ...
  if (Math.abs(v - rounded) <= 1e-9 * Math.max(1, Math.abs(v)))
...
function computeSolveInsertion(mathContent, separator) {
...
var LatexSuiteExtension = class extends import_obsidian7.Plugin {
  ...
  onunload() {
    ...
    (_b = this.fontStyleEl) == null ? void 0 : _b.remove();
    this.fontStyleEl = void 0;
    (_c = this.keyboardSwitcher) == null ? void 0 : _c.destroy();
  ...
  applyRtlFonts() {
    var _a;
    (_a = this.fontStyleEl) == null ? void 0 : _a.remove();
    this.fontStyleEl = void 0;
  ...
  async loadSettings() {
    ...
    this.settings.rtlTextCommands = Array.isArray(data == null ? void 0 : data.rtlTextCommands) ? data.rtlTextCommands : [...DEFAULT_SETTINGS.rtlTextCommands];

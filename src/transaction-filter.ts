...
/**
 * Inner text for a cleaned-up "$...$": trims the padding spaces, but keeps
 * "$ $" instead of collapsing an empty math to "$$", which Obsidian would
 * misread as a display math delimiter. The remaining "$ $" is still rendered
 * as (empty) math by the tolerant math module.
 */
function cleanedInner(text: string): string {
    return text.trim() || " ";
}


export const makeTransactionFilter = (plugin: LatexSuiteExtension): Extension => {
    return EditorState.transactionFilter.of(tr => {
        ...
        if (userEvent === 'input') {
            ...
            if (plugin.settings.builtinSnippets) {
                ...
                if (snippetSpec) return snippetSpec;
            }
